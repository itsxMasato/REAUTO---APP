...
namespace REAUTO_APP
{
    public partial class Solicitudes_Aprob : Form
    {
                            ...
                            using (PdfReader reader = new PdfReader(filename))
                            {
                                using (FileStream fs = new FileStream(nuevoPDF, FileMode.Create, FileAccess.Write))
                                {
                                    using (PdfStamper stamper = new PdfStamper(reader, fs))
                                    {
                                        ...
                                        formulario.SetField("text_1froe", TxtEmpleado.Text);
                                        formulario.SetField("text_4ooxc", TxtDNI.Text);
                                        formulario.SetField("text_2yujc", TxtFInicio.Text);
                                        formulario.SetField("text_3waet", TxtFFinal.Text);
                                        formulario.SetField("text_5sooe", TxtEmpleado.Text);
        ...
        private void dtgvSolisAprob_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            TxtFInicio.Text = dtgvSolisAprob.CurrentRow.Cells[2].Value.ToString();
            TxtFFinal.Text = dtgvSolisAprob.CurrentRow.Cells[3].Value.ToString();
        }
